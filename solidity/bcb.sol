...
contract WorldCup is Context, IERC20, Ownable {
    ...
    string private _name = "World Cup";
    string private _symbol = "WC";
    uint8 private _decimals = 9;

    address payable public marketingWalletAddress = payable(0xeB73be7F102C43D348906Cf8374CFC70Df256056); 
    address payable public NFTWalletAddress = payable(0xe910eB4cde4e4c52362B5A7DD79F50e0F652286F); 
    ...
    constructor () {
        ...
        _totalTaxIfBuying = _buyLiquidityFee.add(_buyMarketingFee).add(_buyNFTFee);
        _totalTaxIfSelling = _sellLiquidityFee.add(_sellMarketingFee).add(_sellNFTFee);
        _totalDistributionShares = _liquidityShare.add(_marketingShare).add(_nftShare);
    ...
    function setDistributionSettings(uint256 newLiquidityShare, uint256 newMarketingShare, uint256 newNFTShare) external onlyOwner() {
    ...
    function setTradeStatus(bool newValue) external onlyOwner {
       tradeOpen = newValue;
    }
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        uint256 amountBNBNFT = amountReceived.mul(_nftShare).div(totalBNBFee);
        uint256 amountBNBMarketing = amountReceived.sub(amountBNBLiquidity).sub(amountBNBNFT);
